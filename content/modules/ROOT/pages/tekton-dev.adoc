...
| `on-push.yaml`
| `git push`
| Builds image → scans for CVEs → signs → deploys to dev environment
...
- `tasks/` - Individual steps (e.g., `build-container.yaml`, `acs-image-scan.yaml`)
...
| `.tekton/on-push.yaml`
| `init` → `clone-repository` → `verify-commit` → `package` → `build-container` → `upload-sboms` + `update-deployment` (parallel) → `acs-image-check` + `acs-image-scan` + `acs-deploy-check` (parallel) → `show-sbom` + `summary`
