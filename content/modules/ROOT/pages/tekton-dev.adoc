...
* **Location**: `{gitlab_url}/development/my-quarkus-tkn[^]`
...
* **Location**: `{gitlab_url}/development/my-quarkus-tkn-gitops[^]`
...
* Something interesting happens - you're prompted for signed commit authentication. You need to follow the browser flow to cryptographically sign your commit:
  - Click on the browser link that appears in your terminal
...
Password: {rhdh_user_password}
----
  - Copy the authentication code from the browser
+
image::tekton-dev-16.png[]
  - Paste the code back into your terminal to complete the signing process
