...
* Check the updated deployment manifests in the `components/qrks-jnk-{user}/overlays/stage` directory
* In the `deployment-patch.yaml` file, notice how the image tag has been automatically updated to your release
* Observe that this GitOps integration happens seamlessly within Jenkins workflows

**Step 3: Validate Staging Environment Success**

In the background, ArgoCD automatically detects the Git changes made by your Jenkins pipeline. ArgoCD then syncs these changes to the staging environment and deployment completes successfully without manual intervention from Jenkins or kubectl commands.

* In the `Topology` tab in Developer Hub, locate the stage application `qrks-jnk-{user}-stage` and click on the 'Open URL' icon (the arrow on the top right of your application) to open the route.
